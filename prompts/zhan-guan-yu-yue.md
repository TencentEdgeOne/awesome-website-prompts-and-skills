# 展馆预约

> **赛道**：Prompt　**作者**：厚重的中国文物 · [GitHub @Yuanyi](https://github.com/Yuanyi)
>
> WorkBuddy × Tencent EdgeOne AI Prompts × Skills 挑战赛 参赛作品

## 🎬 Demo

![展馆预约 cover](../assets/demos/zhan-guan-yu-yue.png)

> 🎞️ 想看动效？[点击查看 demo 动图（GIF）](../assets/demos/zhan-guan-yu-yue.gif)

## 📌 作品信息

| 字段 | 内容 |
|---|---|
| 作品名称 | 展馆预约 |
| 赛道 | Prompt |
| 作者 | 厚重的中国文物 |
| GitHub | [@Yuanyi](https://github.com/Yuanyi) |

## 📝 作品介绍

用于展示中国历史文化的底蕴，并且可以给与展馆进行预约

---

## 🚀 完整 Prompt（直接复制以下内容喂给 AI 编程工具）

```
模块	核心要点
项目定位	情感基调 + 视觉方向 + 明确排除项
技术栈	精确到版本号（React 18 + Vite 5 + TS）
色彩系统	具体到 HSL 值（--primary: 25 55% 50%）
字体规范	字体名 + 用途 + 权重
布局规则	具体数值（max-w: 1280px）+ 禁则
组件规范	CSS 类名 + 效果描述
页面结构	区块数量 + 每块详细规格
媒体资源	CDN 链接或具体来源
后端能力	API 端点 + 方法 + 返回格式
交付标准	功能/响应式/性能验收清单

Install this skill: https://github.com/TencentEdgeOne/edgeone-pages-skills, then deploy to EdgeOne Pages.

```
